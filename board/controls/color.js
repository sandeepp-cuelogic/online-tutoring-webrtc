DrawingBoard.Control.Color = DrawingBoard.Control.extend({
	...
	initialize: function() {
		...
		this.$el.on('click', '.drawing-board-control-colors-picker', function(e) {
	    	...
	    	globColor = color.match(/^rgba?[\s+]?\([\s+]?(\d+)[\s+]?,[\s+]?(\d+)[\s+]?,[\s+]?(\d+)[\s+]?/i);
			 globColor = (globColor && globColor.length === 4) ? "#" +
			  ("0" + parseInt(globColor[1],10).toString(16)).slice(-2) +
			  ("0" + parseInt(globColor[2],10).toString(16)).slice(-2) +
			  ("0" + parseInt(globColor[3],10).toString(16)).slice(-2) : '';
